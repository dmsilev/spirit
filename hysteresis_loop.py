...
cfgs = [f"input/LHF_DDI_glass_14_{str(s)}.cfg" for s in spin_concentrations]
...
def plot_loop(concentration):
    with state.State(f"input/LHF_DDI_glass_14_{str(concentration)}.cfg") as p_state:
        ...
        for i,H in enumerate(fields_hyst):
            ...
            print(f'{H:.3f}', str(concentration))
            ...
            for j in range(converge_max) :
                simulation.start(p_state, simulation.METHOD_MC, solver_type=simulation.SOLVER_VP, single_shot=False)
                simulation.stop(p_state)
                if j == 0 :
     ...
     #               ratio = abs((m_temp-m_prev)/m_prev)
        ...
        plt.plot(fields_hyst, mz[:, 0]/(concentration/100), label=str(concentration/100))
...
if __name__ == '__main__':
    ...
    with mp.Pool(processes=mp.cpu_count()) as pool:
        coercive_fields = pool.map(plot_loop, spin_concentrations)

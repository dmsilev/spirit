...
# cfgs = [f"input/LHF_DDI_glass_14_{str(s)}_coerc.cfg" for s in spin_concentrations]

#with state.State("input/test_Ising_largelattice.cfg") as p_state:
coercive_fields = []

def compute_coercive_field(concentration):
    with state.State(f"input/LHF_DDI_glass_14_{str(concentration)}_coerc.cfg") as p_state:
        ...
        for i,H in enumerate(fields_hyst):
            ...
            if H > 0 :
                ...
                hamiltonian.set_field(p_state,-1*H,(0,0,-1))
     ...
     #               ratio = abs((m_temp-m_prev)/m_prev)
            ...
            if m_temp<0:
                #linear interpolation for x-intercept (coercive field)
                H_prev = fields_hyst[i-1]
                m_prev = mz[i-1]
                m = mz[i]
                H_c = H_prev - m_prev * (H - H_prev) / (m - m_prev)  # Linear interpolation
                # coercive_fields.append((concentration/100, H_c))
                return concentration/100, H_c
                #break

        #np.savetxt("output/"+prefix+"mh.csv",np.transpose((fields_hyst,mx[:,0],my[:,0],mz[:,0],iter_count[:,0])),delimiter=',')


        #data = np.transpose((fields_hyst,mx[:,0],my[:,0],mz[:,0],iter_count[:,0]))
...
if __name__ == '__main__':
    spin_concentrations = [15, 25, 35, 45, 55, 65, 75, 85, 95]

    with mp.Pool(processes=3) as pool:
        coercive_fields = pool.map(compute_coercive_field, spin_concentrations)

    x_vals, y_vals = zip(*coercive_fields)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('hysteresis_loop_coercive_fields.png')

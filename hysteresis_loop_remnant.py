...
Hstep_fine = 0.05
Hfine1 = 2
Hfine2 = 0 #Just need field to go down to 0 for finding remnant H

fields = np.arange(Hmax,Hfine1,-1*Hstep_coarse,dtype=float)
fields_hyst = np.append(fields,np.arange(Hfine1,Hfine2,-1*Hstep_fine,dtype=float))
...
def compute_remnant(concentration):
    with state.State(f"input/LHF_DDI_glass_14_{str(concentration)}.cfg") as p_state:
     ...
     #               ratio = abs((m_temp-m_prev)/m_prev)
        ...
        return concentration/100, mz[-1]/ (concentration /100)
                #break
...
if __name__ == '__main__':
    ...
    with mp.Pool(processes=mp.cpu_count()) as pool:
        coercive_fields = pool.map(compute_remnant, spin_concentrations)
    ...
    plt.title('Remnant magnetisation')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('hysteresis_loop_remnant.png')

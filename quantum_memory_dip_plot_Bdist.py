...
def plot_loop(gamma):
    ...
    #Filter out any vacant sites
        ...
        #For plotting B_z distribution around H_relax
        Hts_plot = np.unique(Hts[np.abs(Hts - H_relax) < 3 * H_step])
        ...
        # last_indices = np.append(last_indices, int((Hmax-H_relax)/H_step * 2 + 1)) #To add index of H_relax after only 2 iterations
        # print('last indices:', last_indices)

        chis = {}
        count = 0
        chi_Hrelax_before_relax = []
        df_Bdist = pd.DataFrame()
        ...
        for i,Ht in enumerate(Hts):
            ...
            if i in last_indices and Ht in Hts_plot:
                tqdm.write(f'==========================Hts{Ht}==========================')
                spins = system.get_spin_directions(
                    p_state)  # Get the current spin state to update the DDI fields from the Ewald sum
                spins[:, 2][
                ...
                #Include external + DDI
                total_field_trans = np.sqrt((Ht + DDI_field_x_from_z) ** 2 + DDI_field_y_from_z ** 2)
                ...
                df_Bdist = pd.concat([
                    df_Bdist,
                    pd.DataFrame({
                        'Ht': Ht * np.ones(len(valid_idx)),
                        'DDI_field_z': DDI_field_z_total[valid_idx],
                        'Total_field_trans': total_field_trans[valid_idx]
                    })
                ], ignore_index=True)
            ...
            #     chi_Hrelax_before_relax.append(chi)
            #
            #     continue
    ...
    return chis, chi_Hrelax_before_relax, df_Bdist
...
if __name__ == '__main__':
    ...
    #     flat = [(round(k[0], 2), round(k[1], 2), v, gamma) for result, _, df in results for k, v in result.items()]
    #
    ...
    # # Save raw data
    # df_all.to_csv(
    #     f'Susceptibility_multi_gammas_{dim}_{n_cycles}_per_gamma_{concentration}_anisotropy_0.7_relax_step_{H_relax_steps}_gammas_{-5}_relax_0.8_gamma_{gamma}_relaxed_5.csv',
    #     index=False)
    #
    ...
    # fig.write_html(
    #     f'Susceptibility_multi_gamma_{dim}_{n_cycles}_{concentration}_anisotropy_0.7_relax_step_{H_relax_steps}_gammas_DDI_{-5}_relax_{H_relax}_gamma_{gamma}_relaxed_5.html')
    ...
    #
    # # Save to CSV
    # df_chi_before.to_csv(f"chi_before_relax_{dim}_{n_cycles}_{concentration}_anisotropy_0.7_relax_step_{H_relax_steps}_gammas_DDI_{-5}_relax_{H_relax}_gamma_{gamma}_relaxed_5.csv", index=False)
    #
    # Unpack all df_Bdist from the results
    df_all_Bdist = pd.concat([r[2] for r in results], ignore_index=True)
    df_all_Bdist.to_csv(f'Susceptibility_multi_gammas_{dim}_{n_cycles}_per_gamma_{concentration}_anisotropy_0.7_relax_step_{H_relax_steps}_gammas_{-5}_relax_0.8_gamma_{gamma}_Bdist_5.csv', index=False)

    hist_data = {}
    unique_Hts = sorted(df_all_Bdist["Ht"].unique())
    print(unique_Hts)
    colors = px.colors.qualitative.Plotly

    for Ht in unique_Hts:
        data = df_all_Bdist[df_all_Bdist["Ht"] == Ht]["DDI_field_z"]
    ...
    for i, Ht in enumerate(unique_Hts):
        x, y = hist_data[Ht]
    ...
    fig.write_html(f'Susceptibility_multi_gamma_{dim}_{n_cycles}_{concentration}_anisotropy_0.7_relax_step_{H_relax_steps}_gammas_DDI_{-5}_relax_{H_relax}_gamma_{gamma}_relaxed_Bzdist.html')

    for Ht in unique_Hts:
        data = df_all_Bdist[df_all_Bdist["Ht"] == Ht]["Total_field_trans"]
        counts, bin_edges = np.histogram(data, bins=80, density=True)
        bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])
        hist_data[Ht] = (bin_centers, counts)
    ...
    fig.update_layout(
        title="Total_field_transverse Distribution for Different Ht",
        xaxis_title="Total_field_transverse (T)",
        yaxis_title="Density",
        legend_title="Ht",
    )

    fig.write_html(f'Susceptibility_multi_gamma_{dim}_{n_cycles}_{concentration}_anisotropy_0.7_relax_step_{H_relax_steps}_gammas_DDI_{-5}_relax_{H_relax}_gamma_{gamma}_relaxed_Btdist.html')

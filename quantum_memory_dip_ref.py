...
def save_and_plot_results(results, dim, n_cycles, concentration, anisotropy, gamma):
    ...
    df_all.to_csv(
        get_unique_filename(
            f'Susceptibility_v3_multi_gammas_{dim}_{n_cycles}_per_gamma_{concentration}_anisotropy_{anisotropy}_gamma_{gamma}_ref_1.csv'),
        index=False)
    ...
    fig = px.line(
        ...
        title="Susceptibility χ vs Ht (reference curves)"
    )

    fig.write_html(
        get_unique_filename(
            f'Susceptibility_v3_multi_gamma_{dim}_{n_cycles}_{concentration}_anisotropy_{anisotropy}_gamma_{gamma}_relaxstepzero_{10}ref_1.html'))
...
def compute_chi_vs_Ht(gamma, dim, concentration):
    """
    ...
    with state.State(f"input/LHF_DDI_glass_14_{concentration}_tunnel_{dim}.cfg", quiet=True) as p_state:
        ...
        for i, Ht in enumerate(Hts):
            ...
            if i % 2 == 1: #Only calculate chi if it's last iteration of each unique Ht
...
if __name__ == '__main__':
    ...
    save_and_plot_results(results, dim, n_cycles, concentration, anisotropy, gamma)
